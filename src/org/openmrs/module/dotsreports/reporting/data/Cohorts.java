...
import org.openmrs.module.dotsreports.reporting.definition.TB03CohortDefinition;
...
public class Cohorts {
	...
	*/
	...
	public static CohortDefinition getDefaultedDuringFilter(Date startDate, Date endDate) {
		return getEnteredStateDuringFilter(TbUtil.getProgramWorkflowState(Context.getService(TbService.class).getConcept(TbConcepts.LOST_TO_FOLLOWUP)), startDate, endDate);	
	}
	...
	public static CohortDefinition getHivPositiveDuring(Date startDate, Date endDate) {
		System.out.println("getHivPositive start:" +  new Date());
	...
	// TODO: figure out what obs to look for here--see ticket HATB-358
		public static CohortDefinition getHivTestedDuring(Date startDate, Date endDate) {
			return ReportUtil.getCodedObsCohort(TimeModifier.ANY, 186, startDate, endDate, SetComparator.IN, Context.getService(TbService.class).getConcept(TbConcepts.POSITIVE).getId(),Context.getService(TbService.class).getConcept(TbConcepts.NEGATIVE).getId(),Context.getService(TbService.class).getConcept(TbConcepts.INDETERMINATE).getId());
		}
	...
	public static CohortDefinition getTB03ByDatesAndLocation(Date startDate, Date endDate, Location loc) {
		TB03CohortDefinition tcd = new TB03CohortDefinition();
		tcd.setLocation(loc);
	...
	*/
	...
	public static CohortDefinition getDOTSPatientGroupDuring(Date startDate, Date endDate, String[] answerConceptMapping) {
		/*return ReportUtil.getCodedObsCohort(TimeModifier.ANY, Context.getService(TbService.class).getConcept(TbConcepts.PATIENT_GROUP).getId(), 
			startDate, endDate, SetComparator.IN, Context.getService(TbService.class).getConcept(answerConceptMapping[0]).getConceptId());*/
		
		StringBuilder q = new StringBuilder();
		q.append("select 	o.person_id ");
		q.append("from		obs o ");
		q.append("where		o.concept_id = " + Context.getService(TbService.class).getConcept(TbConcepts.PATIENT_GROUP).getId() + " ");
		
		q.append("and		o.value_coded = " + Context.getService(TbService.class).getConcept(answerConceptMapping).getId() + " ");
	...
	public static CohortDefinition getMDRPatientGroupDuring(Date startDate, Date endDate, String[] answerConceptMapping ) {
		/*return ReportUtil.getCodedObsCohort(TimeModifier.ANY, Context.getService(TbService.class).getConcept(TbConcepts.CAT_4_CLASSIFICATION_PREVIOUS_TX).getId(), 
			startDate, endDate, SetComparator.IN, Context.getService(TbService.class).getConcept(answerConceptMapping[0]).getConceptId());*/
		
		StringBuilder q = new StringBuilder();
		q.append("select 	o.person_id ");
		q.append("from		obs o ");
		q.append("where		o.concept_id = " + Context.getService(TbService.class).getConcept(TbConcepts.CAT_4_CLASSIFICATION_PREVIOUS_TX).getId() + " ");
